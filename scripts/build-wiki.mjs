...
function build() {
  ...
  execAndLog(`cp -r ${docsFolder}/public/ ${folderToServe}`, { cwd: repoFolder });
  // try copy some static assets, don't cause error if some of them been removed by the user
  try {
    // npm run build:public
    execAndLog(`cp ${docsFolder}/tiddlers/favicon.ico ${folderToServe}/favicon.ico`, { cwd: repoFolder });
  } catch (error) {
    console.log(error);
  }
  // npm run build:nodejs2html
  execAndLog(`tiddlywiki ${docsFolder} --build externalimages`, { cwd: docsFolder });
  execAndLog(`tiddlywiki ${docsFolder} --build externaljs`, { cwd: docsFolder });
  // npm run build:sitemap
  execAndLog(`tiddlywiki ${docsFolder} --rendertiddler sitemap sitemap.xml text/plain && mv ${docsFolder}/output ${repoFolder} && mv ${repoFolder}/output/sitemap.xml ${folderToServe}/sitemap.xml`, {
    cwd: docsFolder,
  });
  ...
  execAndLog(`html-minifier-terser -c ${docsFolder}/html-minifier-terser.config.json -o ${htmlMinifyPath} ${repoFolder}/output/index.html`, { cwd: repoFolder });
  ...
  execAndLog(`workbox injectManifest ${docsFolder}/workbox-config.js`, { cwd: docsFolder });
  // npm run build:clean
  // execAndLog(`rm -r ${repoFolder}/output`, { cwd: repoFolder });
  // npm run build:pluginLibrary
  execAndLog(`tiddlywiki ${docsFolder} --output ${folderToServe}/library --build library`, { cwd: repoFolder });
}
build();
